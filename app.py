...
# Functions moved to modules/ai_providers.py
...
# Functions moved to modules/image_processing.py, modules/textures.py, and modules/text_generation.py

# Function moved to modules/image_processing.py


# Function moved to modules/video_generator.py
...
def generate():
    ...
    try:
        ...
        for key, value in request.form.items():
            if key != 'custom_texture_data':  # Skip logging the base64 data
                print(f"{key}: {value}")
        ...
        if 'texture_file' in request.files and request.files['texture_file'].filename:
            ...
            if '.' in uploaded_file.filename and uploaded_file.filename.rsplit('.', 1)[1].lower() in ['jpg', 'jpeg', 'png']:
                try:
                    ...
                    print(f"Saved uploaded texture as {custom_texture_filename}")
                    
                    # Set the background texture to the new file
                    if request.form.get('background_texture') == 'custom':
                        background_texture = custom_texture_filename
                    else:
                ...
                background_texture = request.form.get('background_texture', 'none')
                print("Invalid file extension for uploaded texture")
        else:
            background_texture = request.form.get('background_texture', 'none')
...
def font_file(filename):
    ...
    try:
        # Security: Only allow .ttf and .otf files
        ...
        return "Error serving font file", 500
...
def media_file(filename):
    ...
    try:
        ...
        # If not found, check if it's one of the default textures without extension
...
# Log AI availability when the module is loaded
log_ai_availability()
